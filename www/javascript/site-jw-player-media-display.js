 ...
 */
class SiteJwPlayerMediaDisplay {
	static current_player_id = null;
	static record_interval = 30; // in seconds
	static players = [];
	static show_resume_message = true;
	static resume_video_text =
		'Resume Where You Left Off (%minutes%:%seconds%)';

	static restart_video_text = 'Start From the Beginning';

	constructor(media_id, container_id) {
		...
		this.on_ready_event = new YAHOO.util.CustomEvent(
			'on_ready',
			this,
			true
		);
	...
	getBestQualitySource(player_width, player_height) {

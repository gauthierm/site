...
class SiteTagEntry {
	 ...
	 * @var string
	 */
	static add_text = 'Add Tag';

	/**
	 * Creates a new tag entry widget
	 *
	 * @param {string}            id                         the unique identifier of this entry object.
	 * @param {DS_XHR|DS_JSArray} data_store                 The type of YUI datastore to use for values.
	 * @param {string[]}          initial_selected_tag_array an array of already selected tag strings.
	 * @param {boolean}           allow_adding_tags          Whether new tags can be added.
	 *
	 * @see http://developer.yahoo.com/yui/autocomplete/
	 */
	constructor(id, data_store, initial_selected_tag_array, allow_adding_tags) {
		this.id = id;
	...
	canAddTag(tag_name) {
		var total_tags =
			this.selected_tag_array.length + this.new_tag_array.length;
	...
	getTagElement(tag_name, title, new_tag) {
		...
		if (new_tag) {
			...
			new_node.appendChild(
				document.createTextNode(SiteTagEntry.new_text)
			);
	...
	itemSelected(oSelf, elItem) {
		this.item_selected = true;
	}

	itemUnSelected(oSelf, elItem) {
		this.item_selected = false;
	}

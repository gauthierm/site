...
class SiteJwPlayerMediaDisplay extends SwatControl
{
    ...
    {
        $valid_fits = array('none', 'exactfit', 'uniform', 'fill');
    ...
    protected function getJavascript()
    {
        ...
        if ($this->media->getInternalValue('scrubber_image') !== null) {
            $javascript .= sprintf(
                "\t%s.vtt_uri = %s;\n",
                $this->getJavascriptVariableName(),
                SwatString::quoteJavaScriptString($this->getVttUri())
            );
        }
        ...
        $javascript .= sprintf(
            ...
            SwatString::quoteJavascriptString(
                $this->getBrowserNotSupportedMessage($this->valid_mime_types)
            )
        );

        if ($this->display_on_complete_message_on_load) {
            $javascript .= sprintf(
                "\t%s." . "display_on_complete_message_on_load = true;\n",
                $this->getJavascriptVariableName()
            );
        }

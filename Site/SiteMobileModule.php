...
class SiteMobileModule extends SiteApplicationModule
{
    ...
    public function autoRelocateMobile($source = '')
    {
        ...
        if ($has_mobile_access) {
            if (isset($_GET[$this->switch_get_var])) {
                ...
                !isset($this->app->cookie->mobile_enabled) ||
        ...
        if ($this->isMobileUrl()) {
            if ($has_mobile_access) {
                ...
                $this->app->relocate(
                    $this->app->getSwitchMobileLink(false, $source)
                );
            }
        } elseif (
            $has_mobile_access &&
            ...
            $this->app->relocate(
                $this->app->getSwitchMobileLink(true, $source)
            );
        }
    }
    ...
    public function getViewport($width = 980)
    {
        ...
        if ($this->isHandheld()) {
            ...
            $meta->content =
                'width=' . intval($width) . ', ' . 'maximum-scale=1.0';
        }

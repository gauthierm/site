...
class SiteAccountSessionModule extends SiteSessionModule
{
    ...
    public function init()
    {
        ...
        // try to log in.
        if (
            $config->account->persistent_login_enabled &&
            !$this->isLoggedIn() &&
            isset($cookie->login)
        ) {
    ...
    public function loginByAccount(
        ...
        $now = new SwatDate();
        $now->toUTC();
        $this->account->updateLastLoginDate($now, $this->app->getRemoteIP(15));
    ...
    public function isAccountDirty()
    {
        ...
        if ($this->isLoggedIn()) {
            ...
            $is_dirty = SwatDB::queryOne($this->app->db, $sql, array(
                'boolean'
            ));
        }
     ...
     */
    public function registerDataObject($name, $class, $destroy_on_logout = true)
    {

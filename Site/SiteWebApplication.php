...
class SiteWebApplication extends SiteApplication
{
    ...
    public function run()
    {
        ...
        if (
            $this->config->memcache->page_cache &&
    ...
    protected function isRequestPageCacheable()
    {
        ...
        $session_is_active =
            $this->hasModule('SiteSessionModule') &&
    ...
    protected function getPageCacheExpirationTime()
    {
        ...
        $timeout =
            $config->page_cache_timeout === null
                ? 900
                : $config->page_cache_timeout;
    ...
    public function getAdminBaseHref()
    {
        ...
        if ($this->config->uri->admin_base != '') {
            ...
            if (mb_substr($admin_base_href, 0, 1) == '/') {
                $admin_base_href =
                    $this->getProtocol($secure) .
                    $this->getServerName($secure) .
                    $admin_base_href;
            }
        }
        ...
        foreach ($this->getSecureSourceList() as $pattern) {
            ...
            if (preg_match($regexp, $source) === 1) {
                if ($this->secure) {
                    ...
                    if (
                        $this->hasSession() &&
                        isset($this->session->_regenerate_id)
                    ) {

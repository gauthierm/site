...
class SiteMedia extends SwatDBDataObject
{
    ...
    public function getFormattedDuration()
    {
        ...
        if ($hours !== null) {
            ...
            if ($minutes !== null) {
                ...
                $format = Site::ngettext('%1$s hour', '%1$s hours', $hours);
            }
        } elseif ($minutes !== null) {
            // 30:30
    ...
    public function getEncodingBinding($encoding_shortname)
    {
        ...
        foreach ($this->encoding_bindings as $binding) {
            $id =
                $binding->media_encoding instanceof SiteMediaEncoding
                    ? $binding->media_encoding->id
    ...
    public function load($id)
    {
        ...
        if (
            ...
            throw new SiteException(
                'Trying to load media with the wrong ' .
    ...
    protected function getSerializableSubDataObjects()
    {
        return array('media_set', 'encoding_bindings');
    }
    ...
    protected function deleteCdnFiles()
    {
        foreach ($this->media_set->encodings as $encoding) {
            ...
            if (
                $binding instanceof SiteMediaEncodingBinding &&
                $binding->on_cdn == true
            ) {
        ...
        if ($operation == 'copy' || $operation == 'update') {
            ...
            $task->override_http_headers = serialize(array(
                'Content-Disposition' => sprintf(
                    ...
                    $this->getContentDispositionFilename($encoding->shortname)
                )
            ));
        } else {

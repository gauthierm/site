...
class SiteVideoMedia extends SiteMedia
{
    ...
    public function getSmallestVideoEncodingBinding()
    {
        ...
        foreach ($this->encoding_bindings as $binding) {
            ...
            if (
                ($smallest === null && $binding->width !== null) ||
    ...
    public function getMediaPlayer(SiteApplication $app)
    {
        ...
        $secure = $app->isSecure();
        if (
            $app->hasModule('SiteMobileModule') &&
        ...
        if ($this->has_hls) {
            $jwplayer->addSource(
                $app->cdn->getUri($this->getHlsFilePath(), $expires, $secure)
            );
        }
    ...
    {
        $count =
            $this->scrubber_image_count > 0
                ? $this->scrubber_image_count
                : $this->getDefaultScrubberImageCount();
    ...
    {
        if ($this->getUriBase() != '') {
            $path = array($this->getUriBase(), $this->path_key);

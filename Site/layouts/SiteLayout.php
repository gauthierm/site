...
class SiteLayout extends SiteObject
{
    ...
    public function clear($name)
    {
        if (!isset($this->data->$name)) {
            throw new SiteException(
                "Layout data property '{$name}' does not " .
    ...
    public function init()
    {
        ...
        $this->data->site_title = SwatString::minimizeEntities(
            $this->app->config->site->title
        );

        if (isset($this->app->config->site->meta_description)) {
            $this->data->meta_description = SwatString::minimizeEntities(
    ...
    protected function completeHtmlHeadEntries()
    {
        ...
        // get combine option
        $combine = $resources->combine && $this->getCombineEnabledByFlagFile();

        // get minify option
        $minify = $resources->minify && $this->getMinifyEnabledByFlagFile();
    ...
    protected function getCombineEnabledByFlagFile()
    {
        ...
        $filename =
            $www_root . DIRECTORY_SEPARATOR . Concentrate_FlagFile::COMBINED;
    ...
    protected function getCompileEnabledByFlagFile()
    {
        $www_root = dirname($_SERVER['SCRIPT_FILENAME']);
        $filename =
            $www_root . DIRECTORY_SEPARATOR . Concentrate_FlagFile::COMPILED;
    ...
    protected function getMinifyEnabledByFlagFile()
    {
        $www_root = dirname($_SERVER['SCRIPT_FILENAME']);
        $filename =
            $www_root . DIRECTORY_SEPARATOR . Concentrate_FlagFile::MINIFIED;

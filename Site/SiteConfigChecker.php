    ...
    private function check()
    {
        ...
        foreach ($ini_array as $section_name => $section_values) {
            ...
            foreach ($section_values as $name => $value) {
                if (
                    !array_key_exists($section_name, $this->definitions) ||
                    !array_key_exists($name, $this->definitions[$section_name])
                ) {
                    $this->debug(
                        sprintf(
                            Site::_("Error: in [%s], '%s' is not defined.\n"),
                            $section_name,
                            $name
                        )
                    );

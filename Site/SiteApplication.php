...
abstract class SiteApplication extends SiteObject
{
    ...
    public function getConfigSetting($setting, SiteInstance $instance = null)
    {
        ...
        if (
            ...
            if (!isset($config_cache[$instance->id])) {
                $config_cache[$instance->id] = SwatDB::getOptionArray(
                    $this->db,
    ...
    {
        $client = new Raven_Client($config->sentry->dsn, array(
    ...
    public function __get($name)
    {
        ...
        throw new SiteException(
            'Application does not have a property with ' .
    ...
    public function getCacheRecordset($key, $wrapper_class, $name_space = null)
    {
        ...
        if ($indexes !== false) {
            ...
            if (count($indexes) > 0) {
                $dataobject_array = $this->getCacheValue($indexes, $name_space);
    ...
    public static function initVar($name, $default = null, $types = 0)
    {
        ...
        if (($types & self::VAR_POST) != 0 && array_key_exists($name, $_POST)) {
            $var = $_POST[$name];
        } elseif (
            ($types & self::VAR_GET) != 0 &&
            array_key_exists($name, $_GET)
        ) {
            $var = $_GET[$name];
        } elseif (
            ($types & self::VAR_REQUEST) != 0 &&
            array_key_exists($name, $_REQUEST)
        ) {
            $var = $_REQUEST[$name];
        } elseif (
            ($types & self::VAR_COOKIE) != 0 &&
            array_key_exists($name, $_COOKIE)
        ) {
            $var = $_COOKIE[$name];
        } elseif (
            ($types & self::VAR_SERVER) != 0 &&
            array_key_exists($name, $_SERVER)
        ) {
            $var = $_SERVER[$name];
        } elseif (
            ($types & self::VAR_SESSION) != 0 &&
            array_key_exists($name, $_SESSION)
        ) {
            $var = $_SESSION[$name];
        } elseif (
            ($types & self::VAR_FILES) != 0 &&

...
class SiteCommentFilter
{
    ...
    protected static $tags = array(
        'a' => array(
            'tag' => 'a',
            'self_closing' => false,
            'attributes' => array('title="[^"]+?"', 'href="http[^"]+?"')
        ),
        'em' => array(
            'tag' => 'em',
            'self_closing' => false,
    ...
    protected static function parseInternal($comment, $strip_invalid_tags)
    {
        ...
        foreach ($matches as $match) {
            ...
            if ($match[0][1] !== $offset) {
                $data = self::getByteSubstring($comment, $offset, $match[0][1]);
            ...
            foreach (self::$tags as $tag) {
                if ($tag['self_closing'] === false) {
                    ...
                    if (
                        array_key_exists($tag['tag'], $match) &&
                        $match[$tag['tag']][1] != -1
                    ) {
                        self::startTag($match[0][0], $tag['tag']);
                    } elseif (
                        array_key_exists('n' . $tag['tag'], $match) &&
                        $match['n' . $tag['tag']][1] != -1
                    ) {
    ...
    protected static function getExpression()
    {
        ...
        foreach (self::$tags as $tag) {
            ...
            if ($tag['self_closing']) {
                ...
                $tag_tokens[] = sprintf('<\/(?P<n%1$s>%1$s)>', $tag['tag']);
            }
        }

...
class SiteAnalyticsModule extends SiteApplicationModule
{
    ...
    public function init()
    {
        ...
        $this->pardot_account_id = $config->analytics->pardot_account_id;

        $this->pardot_campaign_id = $config->analytics->pardot_campaign_id;

        $this->friendbuy_account_id = $config->analytics->friendbuy_account_id;

        $this->vwo_account_id = $config->analytics->vwo_account_id;

        $this->hotjar_account_id = $config->analytics->hotjar_account_id;
...
JS;
            ...
            $javascript = sprintf($javascript, $commands);
        }
...
JS;
    ...
    protected function initGoogleAnalyticsCommands()
    {
        ...
        $this->ga_commands = array(
            array('_setSiteSpeedSampleRate', 100),
            '_trackPageview'
        );
    }
...
JS;
    ...
    public function getFacebookPixelCommandsInlineJavascript()
    {
        ...
        if (
            ...
            // further commands.
            $javascript = $this->getFacebookPixelCommand(array(
                'init',
    ...
    protected function initFacebookPixelCommands()
    {
        // Default commands for all sites:
        // * Track the page view.
        $this->facebook_pixel_commands = array(array('track', 'PageView'));
    }
    ...
    public function hasTwitterPixel()
    {
        return $this->twitter_track_pixel_id != '' &&
            !$this->analytics_opt_out &&
            $this->analytics_enabled;
    }
...
JS;
        ...
        return sprintf($javascript, $twitter_functions);
    }
...
JS;
    ...
    public function getBingUETCommandsInlineJavascript()
    {
        ...
        if ($this->hasBingUET() && count($this->bing_uet_commands) > 0) {
    ...
    protected function getBingUETCommand($command)
    {
        ...
        return sprintf('window.uetq.push(%s);', json_encode($command));
    }
...
JS;
        ...
        return sprintf($javascript, $this->hotjar_account_id);
    }

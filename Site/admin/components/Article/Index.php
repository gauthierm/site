...
class SiteArticleIndex extends AdminIndex
{
    ...
    protected function initArticle()
    {
        ...
        if ($this->id != '' && !$this->article->load($this->id)) {
            throw new AdminNotFoundException(
                sprintf(Site::_('Article with id ‘%s’ not found.'), $this->id)
            );
        }
    }
    ...
    protected function buildInternal()
    {
        ...
        if ($this->id != 0) {
            ...
            $this->ui->getWidget('articles_new')->title = Site::_(
                'New Sub-Article'
            );
        ...
        $this->ui
            ->getWidget('articles_toolbar')
            ->setToolLinkValues($tool_value);

        $this->ui
            ->getWidget('visibility')
            ->addOptionsByArray(SiteArticleActionsProcessor::getActions());
    }
    ...
    protected function buildDetails()
    {
        ...
        $modified_date_renderer = $modified_date_field->getRendererByPosition();
        ...
        if ($this->article->bodytext !== null) {
            $this->article->bodytext = SwatString::condense(
                SwatString::toXHTML($this->article->bodytext)
            );
        }

        if ($this->article->description !== null) {
            $this->article->description = SwatString::condense(
                SwatString::toXHTML($this->article->description)
            );
        }

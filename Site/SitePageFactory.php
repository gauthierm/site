...
abstract class SitePageFactory
{
    ...
    protected function instantiatePage(
        ...
        if (!$this->isPage($class)) {
            throw new SiteClassNotFoundException(
                sprintf(
                    'The provided page ' . 'class ‘%s’ is not a SitePage.',
                    $class
                ),
                0,
                $class
    ...
    protected function instantiateLayout($class, $filename = null)
    {
        if (!$this->isLayout($class)) {
            throw new SiteClassNotFoundException(
                sprintf(
                    'The provided layout ' . 'class ‘%s’ is not a SiteLayout.',
                    $class
                ),
                0,
                $class
    ...
    {
        $this->loadClass($class, $this->page_class_path, $this->page_class_map);
    }
    ...
    protected function isLayout($class)
    {
        ...
        return $class === 'SiteLayout' || is_subclass_of($class, 'SiteLayout');
    }
    ...
    protected function decorate(SiteAbstractPage $page, $class)
    {
        if (!$this->isDecorator($class)) {
            throw new SiteClassNotFoundException(
                sprintf(
                    'The provided page ' .
                        'decorator class ‘%s’ is not a SitePageDecorator.',
                    $class
                ),
                0,
                $class
